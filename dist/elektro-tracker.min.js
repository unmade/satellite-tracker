...
TRACKER.namespace("Player"),TRACKER.Player=function(){"use strict";function e(e){var a=e||{};return f.date=a.date||new Date,f.width=$(document).width(),f.onDateChangeCallback=a.onDateChangeCallback||null,f.toggleCoordinatesCallback=a.toggleCoordinatesCallback||null,f.changeCameraViewCallback=a.changeCameraViewCallback||null,f.snapshotCallback=a.snapshotCallback||null,f.updateScale(f.range,f.width),f.updateDateRange(f.date),f.updateHover(),$("#play").click(f.togglePlay),$("#rewind").click(f.rewind),$("#forward").click(f.forward),$("#now").click(f.toNow),$("#speed").click(f.toggleSpeed),$("#coordinate_system").click(f.toggleCoordinates),$("#camera_view").click(f.showCameraViewPopup),$("#camera_view_popup .st-popup-item").click(f.changeCameraViewCallback),$("#snapshot").click(f.snapshotCallback),$(".st-progress-bar").mousedown(f.onDateClicked).mousemove(f.onDateChange).mouseenter(f.showPrehover).mouseleave(f.hidePrehover),$(document).mousemove(f.onDateClickedAndMove).mouseup(f.dateClicked).mouseup(f.hideCameraViewPopup),f}function a(e){f.clicked=!1}function t(){f.date.setUTCMonth(f.date.getUTCMonth()+1),f.updateDateRange(f.date),f.play||"function"!=typeof f.onDateChangeCallback||f.onDateChangeCallback()}function o(e){var a=$("#camera_view_popup");a.is(e.target)||0!==a.has(e.target).length||a.hide()}function n(){f.clicked||($(".st-hover-progress").hide(100),$(".st-selected-date").hide(100))}function l(e){var a=new Date(f.startDate+f.scale*e.pageX);$(".st-selected-date").css({left:f.width/2>=e.pageX?e.pageX:"auto",right:f.width/2<e.pageX?f.width-e.pageX:"auto"}).text(a),$(".st-hover-progress").css({width:e.pageX}),f.clicked&&($(".st-play-progress").css({width:e.pageX}),f.date=a,f.play||"function"!=typeof f.onDateChangeCallback||f.onDateChangeCallback())}function c(e){f.clicked=!0,f.onDateChange(e)}function i(e){f.clicked&&f.onDateChange(e)}function d(){f.date.setUTCMonth(f.date.getUTCMonth()-1),f.updateDateRange(f.date),f.play||"function"!=typeof f.onDateChangeCallback||f.onDateChangeCallback()}function s(){var e=$("#camera_view_popup"),a=$("#camera_view");e.width()/2;$(".st-popup-arrow").css({left:$(document).width()-a.offset().left+a.outerWidth()/2-5}),e.toggle(100)}function r(){$(".st-hover-progress").show(100),$(".st-selected-date").show(100)}function g(){f.play=!f.play,$("#pause_button").toggleClass("hide"),$("#play_button").toggleClass("hide")}function p(){$(this).toggleClass("warning"),f.coordinateSystem="heliocentric"===f.coordinateSystem?"geocentric":"heliocentric","function"==typeof f.toggleCoordinatesCallback&&f.toggleCoordinatesCallback()}function u(){$(this).toggleClass("red"),f.speed=1e3===f.speed?3e3:1e3}function h(){f.date=new Date,f.updateDateRange(f.date),f.play||"function"!=typeof f.onDateChangeCallback||f.onDateChangeCallback()}function C(e){return f.startDate=new Date(e.getFullYear(),e.getMonth(),1).getTime(),f.endDate=new Date(e.getFullYear(),e.getMonth()+1,0).getTime(),f.range=f.endDate-f.startDate,f.updateScale(f.range,f.width),f}function w(){$(".st-play-progress").css({width:(f.date.getTime()-f.startDate)/f.scale})}function k(e,a){f.width=a,f.scale=f.range/f.width}var f={};return f.onDateChangeCallback=null,f.changeCameraViewCallback=null,f.clicked=!1,f.coordinateSystem="geocentric",f.date=null,f.play=!0,f.speed=1e3,f.snapshotCallback=null,f.toggleCoordinatesCallback=null,f.width=null,f.init=e,f.dateClicked=a,f.forward=t,f.hideCameraViewPopup=o,f.hidePrehover=n,f.onDateChange=l,f.onDateClicked=c,f.onDateClickedAndMove=i,f.rewind=d,f.showCameraViewPopup=s,f.showPrehover=r,f.toggleCoordinates=p,f.togglePlay=g,f.toggleSpeed=u,f.toNow=h,f.updateDateRange=C,f.updateHover=w,f.updateScale=k,f}();
