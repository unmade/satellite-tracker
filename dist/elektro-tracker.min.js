...
TRACKER.namespace("Player"),TRACKER.Player=function(){"use strict";function e(e){var t=e||{};return w.date=t.date||new Date,w.width=$(document).width(),w.callback=t.callback||null,w.updateScale(w.range,w.width),w.updateDateRange(w.date),w.updateHover(),$("#play").click(w.togglePlay),$("#rewind").click(w.rewind),$("#forward").click(w.forward),$("#now").click(w.toNow),$("#speed").click(w.toggleSpeed),$(".st-progress-bar").mousedown(w.onDateClicked).mousemove(w.onDateChange).mouseenter(w.showPrehover).mouseleave(w.hidePrehover),$(document).mousemove(w.onDateClickedAndMove).mouseup(w.dateClicked),w}function t(e){w.clicked=!1}function a(){w.date.setUTCMonth(w.date.getUTCMonth()+1),w.updateDateRange(w.date),w.play||"function"!=typeof w.callback||w.callback()}function n(){w.clicked||($(".st-hover-progress").hide(100),$(".st-selected-date").hide(100))}function o(e){var t=new Date(w.startDate+w.scale*e.pageX);$(".st-selected-date").css({left:w.width/2>=e.pageX?e.pageX:"auto",right:w.width/2<e.pageX?w.width-e.pageX:"auto"}).text(t),$(".st-hover-progress").css({width:e.pageX}),w.clicked&&($(".st-play-progress").css({width:e.pageX}),w.date=t,w.play||"function"!=typeof w.callback||w.callback())}function c(e){w.clicked=!0,w.onDateChange(e)}function l(e){w.clicked&&w.onDateChange(e)}function d(){w.date.setUTCMonth(w.date.getUTCMonth()-1),w.updateDateRange(w.date),w.play||"function"!=typeof w.callback||w.callback()}function s(){$(".st-hover-progress").show(100),$(".st-selected-date").show(100)}function i(){w.play=!w.play,$("#pause_button").toggleClass("hide"),$("#play_button").toggleClass("hide")}function r(){$(this).toggleClass("red"),w.speed=1e3===w.speed?2e3:1e3}function u(){w.date=new Date,w.updateHover()}function g(e){return w.startDate=new Date(e.getFullYear(),e.getMonth(),1).getTime(),w.endDate=new Date(e.getFullYear(),e.getMonth()+1,0).getTime(),w.range=w.endDate-w.startDate,w.updateScale(w.range,w.width),w}function p(){$(".st-play-progress").css({width:(w.date.getTime()-w.startDate)/w.scale})}function h(e,t){w.scale=w.range/w.width}var w={};return w.callback=null,w.clicked=!1,w.date=null,w.play=!0,w.speed=1e3,w.width=null,w.init=e,w.dateClicked=t,w.forward=a,w.hidePrehover=n,w.onDateChange=o,w.onDateClicked=c,w.onDateClickedAndMove=l,w.rewind=d,w.showPrehover=s,w.togglePlay=i,w.toggleSpeed=r,w.toNow=u,w.updateDateRange=g,w.updateHover=p,w.updateScale=h,w}();
